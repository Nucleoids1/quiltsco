<?php
    ...
    {
        error_log('[new] non-committal response: invalid email format for email=' . var_export($_email, true) . ' ip=' . $ip);
    ...
    {
        error_log('[new] non-committal response: rate limit hit for email=' . $_email . ' ip=' . $ip);
        makeCookie('notice', $genericNotice);
        header('Location: ./?s=new_thanks');
        die;
    }
    ...
    {
        error_log('[new] non-committal response: existing account for email=' . $_email . ' ip=' . $ip);
        makeCookie('notice', $genericNotice);
        header('Location: ./?s=new_thanks');
        die;
    }
    ...
    $url = 'https://' . ($number == 1 ? 'www.' : '') . str_replace('www.', '', strtolower(server('HTTP_HOST'))) . '/?s=new_complete&c=' . $cache;
    ...
    sendEmail($_email, 'Quilts Community - Registration', $body);
    error_log('[new] registration email sent for email=' . $_email . ' ip=' . $ip);
    makeCookie('notice', $genericNotice);
    header('Location: ./?s=new_thanks');
    die;
